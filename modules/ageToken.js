export class ageToken extends Token {
    ...
    async _onCreate(tokenData, options, userId) {
        ...
        if (!this.data.bar2?.attribute) {
            const barData = game.settings.get("age-system", "usePowerPoints") ? "powerPoints" : ((this.actor.hasPlayerOwner) && (game.settings.get("age-system", "useConviction")) ? "conviction" : null);
